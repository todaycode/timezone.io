...
const getPasswordValidationError = function(p1, p2) {
  if (!p1 || !p2) return "Sorry, passwords cannot be blank";

  if (p1 !== p2) return "Sorry, the two passwords did not match";

  if (p1.length < 8) return "Passwords must be at least 8 characters";
...
auth.login = function(req, res) {
  if (req.user) return res.redirect(getProfileUrl(req.user));
...
auth.create = function(req, res) {
  ...
  var pwError = getPasswordValidationError(
    req.body.password,
    req.body.password2
  );
  if (pwError) {
      ...
      if (!user && inviteCode) {
        ...
        return UserModel.findOne({ _id: userId }).then(function(user) {
          if (!user || emailHash !== user.getEmailHash())
            return Promise.reject(
              "Sorry, please retry the invite link from your email ;)"
            );
          return user;
        });
      }
    ...
    .then(
      function(user) {
        ...
        newUser.save(function(err) {
          if (err) return renderError(err); // 'Something didn\'t work right there, can you try again?'
      ...
      function(err) {
        renderError(
          err.message || "Something didn't work right there, can you try again?"
        );
      }
    );
};
...
auth.joinTeam = function(req, res) {
  ...
  TeamModel.findOne({ inviteHash: req.params.inviteHash }).then(
    function(team) {
      if (!team) return invalidHashResponse();

      // If the user is logged in, just add them to the team!
      if (req.user) {
        return team.addTeamMember(req.user).then(function(user) {
          var next = req.flash("next");
...
auth.passwordChange = function(req, res) {
  ...
  if (!user.authenticate(req.body.currentPassword)) {
    return res.render("PasswordChange", {
      title: "Change your password",
      errors:
        "Hmm, it doesn't look like that current password matches our records",
      noScript: true
    });
  }

  const pwError = getPasswordValidationError(
    req.body.password,
    req.body.password2
  );
  if (pwError) {
  ...
  user.save(err => {
    if (err)
      return res.render("PasswordChange", {
        title: "Change your password",
        errors:
          "Sorry, We weren't able to change your password at this time, please try again",
        noScript: true
      });
    ...
    req.logIn(req.user, err => {
      ...
      req.flash(
        "message",
        "Awesome, your new password is set and you're ready to go!"
      );
...
auth.passwordResetRequestForm = function(req, res) {
  if (req.user) return res.redirect(req.user.getProfileUrl());
...
auth.passwordResetRequest = function(req, res, next) {
  ...
  UserModel.findOneByEmail(req.body.email).then(
    function(user) {
      if (!user)
        return res.render("PasswordReset", {
          title: "Reset your password",
...
auth.verifyPasswordResetToken = function(req, res, next) {
  ...
  var invalidUrlResponse = function() {
    res.render("PasswordReset", {
      ...
      hideForm: true,
      errors:
        "Sorry, that url seems to be invalid or may have expired after 60 minutes. " +
        "Please try to grab the link from your email again ;)",
      noScript: true
    });
  };

  if (!userId || !resetToken) return invalidUrlResponse();

  UserModel.findOneById(userId).then(
    function(user) {
      if (!user) return invalidUrlResponse();

      var key = user.getPasswordResetKey();
      redisClient.get(key, function(err, token) {
        if (err) return next();

        if (!token || token !== resetToken) return invalidUrlResponse();
  ...
  req.user.save(function(err) {
    if (err)
      return res.render("PasswordReset", {
        title: "Reset your password",
        errors:
          "Sorry, We weren't able to reset your password at this time, please try again",
        noScript: true
      });
